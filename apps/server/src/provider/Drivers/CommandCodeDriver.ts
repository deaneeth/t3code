/**
 * CommandCodeDriver — provider integration for the CommandCode CLI.
 *
 * CommandCode currently exposes a headless JSON event stream rather than an
 * external interactive agent protocol. The adapter therefore keeps a T3
 * session alive while spawning one `command-code -p` process per turn and
 * resumes the returned CommandCode session id on subsequent turns.
 */
...
import {
  ...
  makePendingCommandCodeProvider,
    ...
    Effect.gen(function* () {
      ...
      >({
        ...
        initialSnapshot: (settings) =>
          makePendingCommandCodeProvider(settings.provider).pipe(Effect.map(stampIdentity)),
        checkProvider,
      }).pipe(
        Effect.mapError(
          (cause) =>
            new ProviderDriverError({
              driver: DRIVER_KIND,
              instanceId,
              detail: `Failed to build CommandCode snapshot: ${cause.message ?? String(cause)}`,
              cause,
            }),
        ),
      );
